...
class SoftmaxLoss(nn.Module):
    ...
    def __init__(self,
        ...
        # add cosine similarity + 1:
        self.cosine = nn.CosineSimilarity(dim=1, eps=1e-6)
        # self.classifier = nn.Linear(num_vectors_concatenated * sentence_embedding_dimension, num_labels)
        # self.classifier = nn.Linear(F.relu(nn.Linear(num_vectors_concatenated * sentence_embedding_dimension, 256)),num_labels)
        self.classifier = nn.Sequential(
          nn.Linear(num_vectors_concatenated * sentence_embedding_dimension + 1, 256),
          nn.ReLU(),
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        cosine = self.cosine(rep_a, rep_b).unsqueeze(1)
        vectors_concat.append(cosine)
